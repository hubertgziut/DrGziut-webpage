...
import { CTA, ContactForm, FAQ, Reveal } from './components';
import { common, homeFAQs, nav, surgeryProcedures } from './data';
  ...
  return <>
    <section className="hero">
      ...
      <div className="container hero-content">
        ...
        <Reveal className="reveal-d3"><div className="hero-cta"><Link className="btn btn-solid" to="/konsultacja">{t(common.book)}</Link><Link className="btn btn-ghost" to="/chirurgia-plastyczna">{t(common.explore)}</Link></div></Reveal>
        <div className="hero-badges"><span className="badge">{t({ pl: 'Specjalista chirurgii plastycznej', en: 'Plastic surgery specialist' })}</span><span className="badge">{t({ pl: 'Indywidualny plan leczenia', en: 'Individual treatment planning' })}</span></div>
      </div>
    ...
    <section className="section">
      <div className="container split">
        <Reveal><div className="img-panel" style={{ aspectRatio: '4 / 5', maxHeight: 650 }}><img src="/assets/silk.jpg" alt="" /></div></Reveal>
    ...
    <section className="section section-alt">
      <div className="container">
        ...
        <div className="grid grid-3">
          {surgeryProcedures.slice(0, 6).map((procedure, i) => <Reveal key={i} className={`reveal-d${i % 4}`}><article className="card"><div className="card-num">0{i + 1}</div><h3>{t(procedure.name)}</h3><p>{t(procedure.desc)}</p><Link className="card-link" to="/chirurgia-plastyczna">{t(common.learn)} →</Link></article></Reveal>)}
        </div>
    ...
    <section className="section section-alt">
      <div className="container split">
        <Reveal><div className="img-panel" style={{ aspectRatio: '1.35 / 1' }}><img src="/assets/clinic.jpg" alt={t({ pl: 'Wnętrze kliniki', en: 'Clinic interior' })} /></div></Reveal>
        <Reveal className="reveal-d1"><p className="kicker">{t({ pl: 'O lekarzu', en: 'About the surgeon' })}</p><h2 className="h-section">Hubert Gziut</h2><p style={{ color: 'var(--accent)', fontSize: 13, letterSpacing: '.18em', textTransform: 'uppercase', marginTop: 8 }}>{t({ pl: 'Specjalista chirurgii plastycznej', en: 'Plastic surgery specialist' })}</p>
          ...
          <div className="chip-list" style={{ marginTop: 22 }}><span className="chip">NPWZ 3186517</span><span className="chip">{t({ pl: 'Chirurgia twarzy i powiek', en: 'Facial & eyelid surgery' })}</span><span className="chip">{t({ pl: 'Lifting twarzy i szyi', en: 'Face & neck lift' })}</span></div>
    ...
    <section className="section">
      <div className="container split"><Reveal><div><p className="kicker">{t({ pl: 'Kontakt', en: 'Contact' })}</p><h2 className="h-section">{t({ pl: 'Zacznij od spokojnej konsultacji.', en: 'Start with a considered consultation.' })}</h2><p className="lead" style={{ marginTop: 20 }}>{t({ pl: 'Jeśli nie wiesz, która metoda będzie właściwa, zacznij od rozmowy z lekarzem.', en: 'If you are unsure which method is appropriate, start with a conversation with the physician.' })}</p>
        <div className="chip-list" style={{ marginTop: 28 }}><a className="chip" href="tel:+48533210115">+48 533 210 115</a><a className="chip" href="mailto:kontakt@marinaclinic.pl">kontakt@marinaclinic.pl</a></div></div></Reveal><Reveal className="reveal-d1"><ContactForm /></Reveal></div>
    </section>
  </>;
}
